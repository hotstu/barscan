...
final class DecodeThread extends Thread {
  ...
  DecodeThread(CaptureActivity activity,
    ...
    if (decodeFormats == null || decodeFormats.isEmpty()) {
      ...
      if (prefs.getBoolean(PreferencesConstants.KEY_DECODE_1D_PRODUCT, true)) {
        decodeFormats.addAll(DecodeFormatManager.PRODUCT_FORMATS);
      }
      if (prefs.getBoolean(PreferencesConstants.KEY_DECODE_1D_INDUSTRIAL, true)) {
        decodeFormats.addAll(DecodeFormatManager.INDUSTRIAL_FORMATS);
      }
      if (prefs.getBoolean(PreferencesConstants.KEY_DECODE_QR, true)) {
        decodeFormats.addAll(DecodeFormatManager.QR_CODE_FORMATS);
      }
      if (prefs.getBoolean(PreferencesConstants.KEY_DECODE_DATA_MATRIX, true)) {
